...
OWNER_ID = "6442837812"
bot = telebot.TeleBot('8121743650:AAGr5_Rd6KR_JG0uspYKX-h9UqoVT0WtGJU')
...
def handle_bgmi(message):
    ...
    # In private chats, require valid access unless the user is the owner.
    if message.chat.type == "private" and (caller_id != OWNER_ID):
...
def help_command(message):
    ...
    - **/grant <user_id> <days>** - Grant user access (Owner only).
    - **/revoke <user_id>** - Revoke user access (Owner only).
...
def grant_command(message):
    ...
    try:
        if duration_str.endswith("h"):
            ...
            # If no suffix is provided, assume days
...
def list_users_command(message):
    ...
    for uid, exp in user_access.items():
        ...
        if total_seconds < 0:
            # If access expired, skip or mark as expired
            continue
        days = delta.days
